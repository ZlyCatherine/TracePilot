...
from .base import tracepilot_tool, TracePilotBaseTool
import os
import json
import uuid
import threading
...
def get_system_model_info() -> str:
    """
    获取当前 TracePilot 正在运行的底层大模型（LLM）型号和提供商信息。
...
def read_user_profile() -> str:
    """
    读取用户的全局显性记忆档案。
    当你需要了解已经记录的用户称呼、偏好、习惯或其他长期信息时，先调用此工具。
    如果用户要求你“记住某事”，请先读取当前档案，再调用 save_user_profile 写入融合后的完整档案。
    """
    if not os.path.exists(PROFILE_PATH):
        return "当前还没有任何用户长期画像记录。"

    try:
        with open(PROFILE_PATH, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read().strip()
    except Exception as e:
        return f"读取用户长期画像失败：{str(e)}"
