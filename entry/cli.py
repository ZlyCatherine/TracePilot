...
from tracepilot.core.provider import get_provider
...
app = typer.Typer(help="TracePilot - 赛博智能终端")
console = Console()

trace_style = questionary.Style([
...
def config_wizard():
    console.clear()
    console.print(Panel(
        "👾 Welcome to [bold #8d52ff]TracePilot[/bold #8d52ff]...\n\n☁️[dim] 请完成模型配置，我们将把密钥安全固化在本地。[/dim]", 
        title="[bold white]✦  TracePilot Config[/bold white]", 
        border_style="#8d52ff"
    ))
    ...
    if not provider_raw:
        console.print("[dim #8d52ff]✦   录入中断，TracePilot 配置已取消。[/dim #8d52ff]")
        return
    ...
    console.print(Panel(
        ...
        f"👉 输入 [bold #00ffff]tracepilot run[/bold #00ffff] 即可启动系统！",
        border_style="#00ffff"
    ))

def _show_boot_error():
    console.print(Panel(
        "[bold #00ffff]TracePilot未完成配置![/bold #00ffff]\n\n"
        "[#8d52ff]检测到 API Key、模型或Baseurl。请重新执行以下命令完成配置：[/#8d52ff]\n"
        "[bold #00ffff]tracepilot config[/bold #00ffff]",
...
def run_agent():
    ...
    import entry.main as tracepilot_main
    tracepilot_main.main()

@app.command("monitor")
def run_monitor():    
        
    try:
        import entry.monitor as tracepilot_monitor
        tracepilot_monitor.main()
